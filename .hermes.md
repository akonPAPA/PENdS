# Pends — Project-Level Agent Context

## Safety Boundary

The Pends agent is a penetration testing assistant. The following safety rules are **not negotiable**:
...
- **Use the guard** — Every target-touching command must run through `pends_exec`, `pends_exec_burst`, or a typed adapter. The plugin best-effort blocks clearly target-touching raw `terminal` calls, but this classifier is not scope enforcement. Target scripts must use typed execution. `execute_code` is permitted only with the required first-line Pends metadata header and is recorded as engagement evidence. For validation without execution, call `check-command` with `--eng-dir`, `--target`, and `--session-id`: `python $HOME/.hermes/profiles/pends/scripts/pends_guard.py check-command --scope $ENG_DIR/scope/scope.yaml --eng-dir "$ENG_DIR" --target "<primary-host-or-ip>" --session-id "<session-id>" --phase <PHASE> --command "<cmd>"`. Exit code `1` blocks execution; exit code `2` requires explicit review or approval.
...
Long Pends sessions must not degrade into ad-hoc testing. The authoritative drift guard is in [`skills/pentest/SKILL.md §2`](./skills/pentest/SKILL.md#2-workflow-drift-guard) — this section summarises the key invariants only.
...
- Every target-touching command must pass `check-command` with `--eng-dir`, `--target`, and `--session-id`; typed Pends execution runs this gate internally before execution.
...
Pends **must not** touch a target (no curl, nmap, browser, web_search for the target, etc.) until **all five** artifacts exist. This applies to every engagement and resumed work block.

| # | Artifact | Created by | Verified by |
|---|----------|-----------|-------------|
| 1 | `$ENG_DIR/` (engagement directory) | `pends_guard.py init-engagement` | `pends_guard.py check-bootstrap --eng-dir "$ENG_DIR"` |
| 2 | `$ENG_DIR/scope/scope.yaml` (approved) | `init-engagement`, then fill and approve | `pends_guard.py validate-scope --scope "$ENG_DIR/scope/scope.yaml"` exits 0 |
...
task, call `pends_record_ptt` with its routed `skill`, `technique`, and any
...
`pends_status` or `python $HOME/.hermes/profiles/pends/scripts/pends_guard.py
...
```text
python $HOME/.hermes/profiles/pends/scripts/pends_guard.py check-command --scope $ENG_DIR/scope/scope.yaml --eng-dir "$ENG_DIR" --target "<primary-host-or-ip>" --session-id "<session-id>" --phase <PHASE> --command "<cmd>"
```

**Run-to-completion rule:** after `/goal set` or context compression, continue
in the same conversation, re-read `$ENG_DIR/state/`, and call `pends_status`.
...
- **Explain guard state cheaply** — Call `pends_status` before guessing at a phase or lock failure. It reports the current task/phase, phase requirements, pending commands, blockers, and next actions without mutation.
- **Review each batch once** — After a bounded batch, call `pends_review_batch` with the active PTT id, lifecycle status, and truthful evidence note. Include the optional finding object only when that batch's receipts support the finding. Retries reuse the same batch marker and finding.
- **Handle protected system files explicitly** — If Hermes' built-in file tool refuses a sensitive system path, stage the file under `$ENG_DIR/exploits/` and, only when the approved engagement requires it, use the host terminal with the necessary privilege to install it. Pends has no `grant-writelist` command; do not invent one.
...
- **Delegate** — Use `pends_exec` with `background=true` or `pends_listener` for tracked long-running work. Do not start target-facing background jobs through raw `terminal`.
...
Pends conversations are isolated per project/engagement:
...
Pends is designed for native Kali Linux or Parrot OS first. Its `terminal`
...
- Evidence written to `/engagements/` maps to `engagements/` in the Pends repo
...
Pends runs on **Hermes built-in toolsets** (see [`skills/pentest/SKILL.md §1`](./skills/pentest/SKILL.md#1-operating-model) for the capability inventory and [`README.md §Toolsets`](./README.md#enabled-toolsets) for the full matrix). The canonical command-gate logic lives in the top-level modules under `plugins/pends_guard/` and is callable two ways:

- **CLI:** `python scripts/pends_guard.py check-command ...` (the path documented in SKILL.md §2).
- **Hermes plugin:** `plugins/pends_guard/` registers typed guard tools (`pends_status`, `pends_check_command`, `pends_record_ptt`, `pends_record_hypothesis`, `pends_exec`, `pends_review_batch`, and adapters) that call the shared service directly. The plugin is required for target execution.
...
---
*This file is auto-loaded from the repo root by Hermes as project-level context. It governs Pends's safety, behaviour, and tool usage policies.*
