...
- Fixed hypothesis board rewriter in `plugins/pends_guard/hypotheses.py` to preserve structural sections (`## Observations`, `## Decoy Trail`, `## Research Log`, `## Resolved Theories`).
...
- Added a Duck Store benchmark harness: 4-file suite (`score.py`, `challenges.json`, `scope.yaml`, `engage.md`) to evaluate Pends against escape.tech's Duck Store with repeatable, evidence-gated scoring.
...
- Added model-visible `pends_status` diagnostics, phase-aware 10/20-command sync windows, a 350-iteration profile budget, and atomic `pends_review_batch` reconciliation with optional receipt-backed finding output.
...
- Made the existing `pends_exec` contract explicit for every installed non-interactive Kali/Parrot CLI tool, and removed the partial target-tool name list from raw-terminal classification in favor of generic target-literal detection.
- Reorganised the guard into focused top-level modules under `plugins/pends_guard/`, with separate history, result, execution, state, target, and service responsibilities.
...
- Kept target parsing and target-scope enforcement in `plugins/pends_guard/targets.py`, leaving `command.py` focused on policy orchestration.
- Allowed `pends_record_ptt` to start one untouched phase-bound task, removing the initial active-task deadlock while retaining fail-closed batch reviews.
...
- Consolidated Pends Guard into a Hermes-native plugin.
