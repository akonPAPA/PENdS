...
from plugins.pends_guard import adapters, execution
...
def test_local_executor_records_receipt_and_history(tmp_path):
    ...
    assert "pends-test" in receipt["stdout_preview"]
    assert (eng / receipt["evidence_paths"]["manifest"]).exists()
    assert "echo pends-test" in (eng / "state" / "history.md").read_text(encoding="utf-8")
