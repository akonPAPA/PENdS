"""Regression tests for burst mode (pends_exec_burst) and pends_target.
...
from plugins.pends_guard import (  # noqa: E402
    bootstrap,
    execution,
    ptt,  # noqa: E402
    state,
...
from plugins.pends_guard import handlers as tools  # noqa: E402
from plugins.pends_guard.command import CheckResult  # noqa: E402
from plugins.pends_guard.targets import resolve_target  # noqa: E402
...
def test_relative_engagement_paths_stay_under_profile_root(monkeypatch):
    monkeypatch.delenv("PENDS_ENG_ROOT", raising=False)
...
def _run(*args):
    return subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "pends_guard.py"), *args],
        capture_output=True,
        text=True,
    )
...
def test_exec_burst_preflights_every_command_before_launch(eng, monkeypatch):
    from plugins.pends_guard.handlers import exec_handlers
...
def test_plugin_exposes_new_tools():
    ...
    manifest = yaml.safe_load(
        (ROOT / "plugins" / "pends_guard" / "plugin.yaml").read_text(encoding="utf-8")
    )
    tool_names = set(manifest["provides_tools"])
    assert "pends_exec_burst" in tool_names
    assert "pends_target" in tool_names
    assert "pends_review_batch" in tool_names
    assert (
        not {
            "pends_sync_done",
            "pends_review_and_release",
            "pends_finding",
        }
        & tool_names
    )
