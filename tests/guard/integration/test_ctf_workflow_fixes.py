...
from plugins.pends_guard import adapters, ptt, state
from plugins.pends_guard import handlers as service
...
def test_listener_with_vpn_ip_allowed(ctf_eng, monkeypatch):
    """Verify pends_listener with attacker VPN IP is not blocked as out-of-scope target."""
...
def test_listener_with_ipv6_bind_host_allowed(ctf_eng, monkeypatch):
    """Verify pends_listener with IPv6 loopback bind_host (::1 or [::1]:4444) is allowed."""
    monkeypatch.setattr(adapters, "_installed_netcat_variant", lambda binary: ("nc", "openbsd"))
...
def test_invalid_ptt_start_status_error_message(ctf_eng):
    ...
    from plugins.pends_guard.handlers.ptt_handlers import _start_ptt_task
...
def test_parse_target_token_ipv6_url():
    ...
    from plugins.pends_guard import targets
...
def test_update_hypothesis_merge_existing_fields(ctf_eng):
    ...
    from plugins.pends_guard import hypotheses
...
def test_findings_lowercase_hypothesis_id(ctf_eng, monkeypatch):
    ...
    from plugins.pends_guard import findings, hypotheses
...
def test_terminal_policy_ipv6_target_blocked():
    ...
    from plugins.pends_guard import terminal_policy
