...
# Make `pends_guard` resolvable
_PLUGIN_ROOT = ROOT / "plugins" / "pends_guard"
...
from plugins.pends_guard.targets import extract_target_candidates
...
def _fake_target_executor(monkeypatch):
    ...
    def fake_execute(command, *, eng_dir, phase, **kwargs):
        ...
        from plugins.pends_guard.phases import normalize_phase, suppresses_heartbeat
...
def test_exec_auto_records_history_but_requires_explicit_ptt_review(monkeypatch, tmp_path):
    ...
    from plugins.pends_guard import state as _state
...
def test_message_ticks_are_diagnostic_and_do_not_trigger_heartbeat(monkeypatch, tmp_path):
    ...
    from plugins.pends_guard import _pre_llm_call_hook
