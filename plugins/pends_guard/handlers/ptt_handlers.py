    ...
    for _name, mod in list(sys.modules.items()):
        if mod and hasattr(mod, "HermesSkillViewAdapter"):
            ...
            if getattr(cls, "__module__", "") not in (
                "plugins.pends_guard.skill_receipts",
...
def _validate_record_ptt_inputs(a: dict, doc: list, pending: dict | None):
    ...
    if pending:
        raise ValueError(
            "a target batch is pending; use pends_review_batch instead of pends_record_ptt"
        )
...
def handle_review_batch(a, **kwargs):
    ...
    try:
        ...
        return _json(
            ...
            released=False,
            error=str(exc),
            next_action="Resolve the reported batch, PTT, history, or finding issue and retry pends_review_batch",
        )
