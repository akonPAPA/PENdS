"""Typed tool schemas for the pends-guard plugin using Pydantic v2."""
...
class CheckCommandArgsModel(BaseModel):
    """Run a check-command gate (typed wrapper over pends_guard.py check-command)."""
...
class RecordPttArgsModel(BaseModel):
    ...
    status: str = ""
    note: str = ""
    skill: str = Field(..., description="Selected Pends skill required before task activation")
...
class ExecBurstArgsModel(BaseModel):
    """Single-approval bounded command batch. Requires one unambiguous [~] PTT task. Every completed command is appended to history automatically, but the executor never updates PTT progress. Review the batch once with pends_review_batch. Use for recon and exploit/race batches; never raw terminal for targets."""
...
class HttpxArgsModel(AdapterCommonModel):
    """Run typed HTTP probing through pends_exec."""

    model_config = ConfigDict(extra="forbid")


class NucleiArgsModel(AdapterCommonModel):
    """Run a typed nuclei scan through pends_exec; scanner output remains unconfirmed evidence."""
...
class FfufArgsModel(AdapterCommonModel):
    """Run typed ffuf content discovery through pends_exec."""
...
class StatusArgsModel(BaseModel):
    ...
    eng_dir: str = Field(
        ..., description="engagement dir ($ENG_DIR / $PENDS_ENG_ROOT env also honoured)"
    )
...
HEARTBEAT_DONE_SCHEMA = to_tool_schema(
    ...
    description=(
        ...
        " heartbeat. Clears heartbeat lock so pends_exec may release the next command."
    ),
)
EXEC_BURST_SCHEMA = to_tool_schema(ExecBurstArgsModel, name="pends_exec_burst")
...
TARGET_SCHEMA = to_tool_schema(TargetArgsModel, name="pends_target")
STATUS_SCHEMA = to_tool_schema(StatusArgsModel, name="pends_status")
