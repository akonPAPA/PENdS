...
Pends keeps the built-in ``terminal`` tool available for host-local work, but
raw terminal calls must not become an escape hatch around the typed Pends
...
The typed ``pends_exec`` and ``pends_exec_burst`` tools remain the authoritative
...
def _is_pends_init_command(seg: CommandSegment) -> bool:
    """Return whether ``seg`` invokes Pends's host-local bootstrap command."""
    ...
    for index, word in enumerate(words):
        ...
        if (
            (script == "scripts/pends_guard.py" or script.endswith("/scripts/pends_guard.py"))
...
def _message(reason: str) -> str:
    return (
        "RAW TERMINAL TARGET EXECUTION BLOCKED by Pends: "
        f"{reason}. Use `pends_exec` for one command or `pends_exec_burst` "
