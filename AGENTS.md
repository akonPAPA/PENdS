...
uv run python -m plugins.pends_guard.release
```
...
- **Hermes Runtime Contract:** All target-touching CLI command execution in Pends engagements MUST go through `plugins.pends_guard` typed Hermes tool calls (`pends_exec`, `pends_record_ptt`, `pends_review_batch`, `pends_record_hypothesis`, `pends_target`, `pends_status`, `pends_listener`). Never invoke flat CLI scripts (`python pends_guard.py`) as a substitute for typed tool calls.
...
- **Typed Schemas:** Use Pydantic v2 `BaseModel` models in `plugins/pends_guard/schemas.py` for all tool parameter specifications.
...
1. **NEVER Bypass Target Execution Guards:** Never run target-touching commands directly in raw shell without `pends_exec` or `pends_exec_burst`.
...
3. **NEVER Hardcode Target IPs:** Resolve target hosts dynamically via `pends_target` or `scope.yaml`.
