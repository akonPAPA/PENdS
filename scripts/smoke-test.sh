#!/usr/bin/env bash
# =============================================================================
# smoke-test.sh — Pends Kali/Parrot Release Readiness Check
# =============================================================================
...
python3 scripts/pends_guard.py check-release
...
seed_history_fixture(){
  python3 - "$1" "$2" "$3" <<'PY'
import sys
from plugins.pends_guard import history
...
set +e
unbooted_output=$(python3 scripts/pends_guard.py check-bootstrap --eng-dir "$SMOKE_ENG" 2>&1)
unbooted_exit=$?
set -e
...
set +e
unbooted_cmd=$(python3 scripts/pends_guard.py check-command --scope "$SMOKE_ENG/no-scope.yaml" --phase recon --command "curl http://10.129.245.218" 2>&1)
unbooted_cmd_exit=$?
set -e
...
python3 scripts/pends_guard.py record-ptt --eng-dir "$SMOKE_ENG" --id PT-001 --status "[x]" --note "smoke bootstrap" --skill pentest --technique recon >/dev/null 2>&1
...
set +e
booted_output=$(python3 scripts/pends_guard.py check-bootstrap --eng-dir "$SMOKE_ENG" 2>&1)
booted_exit=$?
set -e
...
set +e
inscope_output=$(python3 scripts/pends_guard.py check-command --scope "$SMOKE_ENG/scope/scope.yaml" --phase recon --command "curl http://10.129.245.218" 2>&1)
inscope_exit=$?
set -e
...
set +e
outofscope_output=$(python3 scripts/pends_guard.py check-command --scope "$SMOKE_ENG/scope/scope.yaml" --phase recon --command "curl http://other.example.com" 2>&1)
outofscope_exit=$?
set -e
...
set +e
ptt_out=$(python3 scripts/pends_guard.py record-ptt --eng-dir "$SMOKE_GUARD" --id PT-001 --status "[x]" --note "smoke test passed" --skill pentest --technique recon 2>&1)
ptt_exit=$?
set -e
...
set +e
bad_ptt=$(python3 scripts/pends_guard.py record-ptt --eng-dir "$SMOKE_GUARD" --id PT-001 --status INVALID --skill pentest --technique recon 2>&1 || true)
bad_ptt_exit=$?
set -e
...
set +e
bad_id=$(python3 scripts/pends_guard.py record-ptt --eng-dir "$SMOKE_GUARD" --id PT-999 --status "[~]" --skill pentest --technique recon 2>&1)
bad_id_exit=$?
set -e
...
for removed in review-and-release finding sync-done record-history message-tick skill-status check-skill-loaded; do
  set +e
  removed_out=$(python3 scripts/pends_guard.py "$removed" --help 2>&1)
  removed_exit=$?
  set -e
...
set +e
stale_out=$(python3 scripts/pends_guard.py check-bootstrap --eng-dir "$STALE_ENG" 2>&1)
stale_exit=$?
set -e
rm -rf "$STALE_ENG"
...
python3 scripts/pends_guard.py record-ptt --eng-dir "$SMOKE_FRESH" --id PT-016 --status "[x]" --note "nmap done" --skill pentest --technique recon >/dev/null 2>&1
...
set +e
fresh_ok=$(python3 scripts/pends_guard.py check-command --scope "$SMOKE_FRESH/scope/scope.yaml" --eng-dir "$SMOKE_FRESH" --session-id fresh --phase recon --command "nmap 10.129.45.113" 2>&1)
fresh_ok_exit=$?
set -e
...
set +e
fresh_noskill=$(python3 scripts/pends_guard.py check-command --scope "$SMOKE_FRESH/scope/scope.yaml" --eng-dir "$SMOKE_FRESH" --phase recon --command "nmap 10.129.45.113" 2>&1)
fresh_noskill_exit=$?
set -e
...
python3 scripts/pends_guard.py record-ptt --eng-dir "$SMOKE_STALEPTT" --id PT-040 --status "[~]" --note "exploiting" --skill pentest --technique exploit-validation --hypothesis-id H-001 >/dev/null 2>&1
...
YAML
set +e
stale_ptt_out=$(python3 scripts/pends_guard.py check-command --scope "$SMOKE_STALEPTT/scope/scope.yaml" --eng-dir "$SMOKE_STALEPTT" --session-id stale --phase exploitation --command "curl 10.129.45.113" 2>&1)
stale_ptt_exit=$?
set -e
...
python3 scripts/pends_guard.py record-ptt --eng-dir "$SMOKE_STALEHYP" --id PT-040 --status "[~]" --note "exploiting" --skill pentest --technique exploit-validation --hypothesis-id H-001 >/dev/null 2>&1
...
YAML
set +e
stale_hyp_out=$(python3 scripts/pends_guard.py check-command --scope "$SMOKE_STALEHYP/scope/scope.yaml" --eng-dir "$SMOKE_STALEHYP" --session-id sh --phase exploitation --command "curl 10.129.45.113" 2>&1)
stale_hyp_exit=$?
set -e
...
# =============================================================================
header "3.8 Canonical Batch Review Lifecycle (pends_review_batch)"
...
python3 scripts/pends_guard.py record-ptt --eng-dir "$GATES_DIR" --id PT-010 --status "[~]" --note "active recon" --skill pentest --technique recon >/dev/null 2>&1
...
from plugins.pends_guard import history, service, state
...
print("    ok: pends_review_batch reviewed PTT and released the batch")
print("GATES_OK")
PY
gates_exit=$?
...
if [ "$SKIP_INSTALL" = true ]; then
  ...
  SMOKE_PROFILE="pends-smoke-$(date +%s)"
  ...
  if hermes -p "$SMOKE_PROFILE" chat -q "Smoke test: reply with 'Pends profile loaded and responding'" -Q 2>&1; then
