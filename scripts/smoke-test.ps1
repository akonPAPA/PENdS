...
$smokeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("pends-smoke-" + [guid]::NewGuid().ToString("N"))
$engagement = Join-Path $smokeRoot "engagement"

function Invoke-Guard {
    param([string[]]$Arguments)
    $output = & $python (Join-Path $repoRoot "scripts\pends_guard.py") @Arguments 2>&1
    if ($LASTEXITCODE -ne 0) {
        throw "pends_guard.py $($Arguments -join ' ') failed with exit code $LASTEXITCODE`n$($output -join "`n")"
    }
    return $output
}
