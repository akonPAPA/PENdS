#!/usr/bin/env python3
"""CLI entry point for pends_guard — delegates to plugins/pends_guard/."""
...
from plugins.pends_guard import bootstrap, command, handlers, state
...
def cmd_check_release(args) -> int:
    from plugins.pends_guard import release
...
def cmd_search_exploit(args: argparse.Namespace) -> int:
    from plugins.pends_guard import adapters
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="pends_guard.py")
