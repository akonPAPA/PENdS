#!/usr/bin/env bash
# Kali container exec helper for the Pends Hermes profile.
#
...
#
# The container mounts /engagements -> ./engagements/ in the Pends repo,
# so evidence and output files written to /engagements/ are accessible
# from the host at <pends-repo-root>/engagements/.
#
# If the container isn't running, starts it automatically.

CONTAINER_NAME="${PENDS_KALI_CONTAINER:-kali-pentest}"
