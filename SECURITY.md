...
Pends is a **defensive security assessment profile** — it helps authorised testers find and document vulnerabilities in systems they have permission to test.

If you discover a security issue in Pends itself (not a target being tested with Pends):
...
- The required `plugins/pends_guard/` execution guard and registered tools
- The `scripts/pends_guard.py` diagnostic/admin entrypoint and release smoke helpers
...
- Vulnerabilities discovered **by Pends** during authorised testing (those go in the engagement report)
- Third-party tools (nmap, sqlmap, etc.) that Pends invokes — report those to their respective projects
